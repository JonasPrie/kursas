		<?php
			include('../app/views/header.php');
		?>

		<div class="col-12 main-page-container">

			<?php
				include('../app/views/menu.php');
			?>

			<div class="col-10 content-wrapper">
				<div class="shipping-container">
					<div class="col-6 shipping-fields-and-text">
						<span>Shipping local</span>
					</div>
					<div class="col-6 shipping-fields-and-text">
						<span>Shipping EU</span>
					</div>
				</div>
				<div class="first-content-block-row first-content-background">
					<div class="first-content-block-text">
						<p>Buckfast Bee Queens: Virgin or Mated</p><br>
						<p id="second-row-text">Carnica Bee Queens: Virgin or Mated</p><br>
						<p id="third-row-text">Various Honey, Pollen and Bee Bread</p>
						<p id="try-our-products-text">Try our products now</p>
					</div>
				</div>
				<div class="all-content-blocks">
					<div class="col-8 blocks-in-all-content-blocks">
						<figure>
							<a href="various-types-honey.php">
								<img src="../app/images/various-types-honey-medium.jpg" alt="Various types of honey" title="Various types of honey">
							</a>
							<figcaption>Various types of honey</figcaption>
						</figure>
						<h2>
							<a href="various-types-honey.php">
								Various types of honey
							</a>
						</h2>
						<p>Different types of honey.</p>
						<p id="wrapper-price-tags-text">Price: 19.50&euro;</p>
						<p id="read-more-text">
							<a href="various-types-honey.php">
								More about Various Types of Honey
							</a>
						</p>
						<div class="col-9 cart-call-buttons-in-content-block-wrapper">
							<div class="col-6 cart-call-button fas fa-shopping-cart"></div>
							<div class="col-6 cart-call-button fas fa-phone"></div>
						</div>
					</div>
					<div class="col-8 blocks-in-all-content-blocks">
						<figure>
							<a href="linden-honey.php">
								<img src="../app/images/linden-honey-medium.jpg" alt="Linden honey" title="Linden honey">
							</a>
							<figcaption>Linden honey</figcaption>
						</figure>
						<h2>
							<a href="linden-honey.php">
								Linden Honey
							</a>
						</h2>
						<p>Tasty and healthy honey.</p>
						<p id="wrapper-price-tags-text">Price: 6.50&euro;</p>
						<p id="read-more-text">
							<a href="linden-honey.php">
								More about Linden Honey
							</a>
						</p>
						<div class="col-9 cart-call-buttons-in-content-block-wrapper">
							<div class="col-6 cart-call-button fas fa-shopping-cart"></div>
							<div class="col-6 cart-call-button fas fa-phone"></div>
						</div>
					</div>
				</div>
				<div class="all-content-blocks">
					<div class="col-8 blocks-in-all-content-blocks">
						<figure>
							<a href="buckwheat-honey.php">
								<img src="../app/images/buckwheat-honey-medium.jpg" alt="Buckwheat honey" title="Buckwheat honey">
							</a>
							<figcaption>Buckwheat honey</figcaption>
						</figure>
						<h2>
							<a href="buckwheat-honey.php">
								Buckwheat Honey
							</a>
						</h2>
						<p>Dark, healthy, full of iron honey.</p>
						<p id="wrapper-price-tags-text">Price: 6.50&euro;</p>
						<p id="read-more-text">
							<a href="buckwheat-honey.php">
								More about Buckwheat Honey
							</a>
						</p>
						<div class="col-9 cart-call-buttons-in-content-block-wrapper">
							<div class="col-6 cart-call-button fas fa-shopping-cart"></div>
							<div class="col-6 cart-call-button fas fa-phone"></div>
						</div>
					</div>
					<div class="col-8 blocks-in-all-content-blocks">
						<figure>
							<a href="forrest-raspberry-honey.php">
								<img src="../app/images/forrest-raspberry-honey-medium.jpg" alt="Forrest and Raspberry honey" title="Forrest and Raspberry honey">
							</a>
							<figcaption>Forrest and Raspberry honey</figcaption>
						</figure>
						<h2>
							<a href="forrest-raspberry-honey.php">
								Forrest and Raspberry Honey
							</a>
						</h2>
						<p>Tasty.</p>
						<p id="wrapper-price-tags-text">Price: 6.50&euro;</p>
						<p id="read-more-text">
							<a href="forrest-raspberry-honey.php">
								More about Forrest and Raspberry honey
							</a>
						</p>
						<div class="col-9 cart-call-buttons-in-content-block-wrapper">
							<div class="col-6 cart-call-button fas fa-shopping-cart"></div>
							<div class="col-6 cart-call-button fas fa-phone"></div>
						</div>
					</div>
				</div>
				<div class="all-content-blocks">
					<div class="col-8 blocks-in-all-content-blocks">
						<figure>
							<a href="present-box-honey.php">
								<img src="../app/images/honey-box-present-medium.jpg" alt="Honey box present" title="Honey box present">
							</a>
							<figcaption>Honey box present</figcaption>
						</figure>
						<h2>
							<a href="present-box-honey.php.php">
								Honey box present
							</a>
						</h2>
						<p>Dark, healthy, full of iron honey.</p>
						<p id="wrapper-price-tags-text">Price: 25.00&euro;</p>
						<p id="read-more-text">
							<a href="present-box-honey.php.php">
								More about Honey Box Present
							</a>
						</p>
						<div class="col-9 cart-call-buttons-in-content-block-wrapper">
							<div class="col-6 cart-call-button fas fa-shopping-cart"></div>
							<div class="col-6 cart-call-button fas fa-phone"></div>
						</div>
					</div>
				</div>
			</div>
		</div>

		<?php
			include('../app/views/footer.php');
		?>

		</div>
	</body>

</html>
